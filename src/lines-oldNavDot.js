...
(function() {
  ...
  */
  ...
  Lines.prototype.navDot2 = function(navData = {}, cb) {
    ...
    navData.x1 = navData.x - (this.cfg.chart.navDot / 2) + 1;
    navData.y1 = navData.y - (this.cfg.chart.navDot / 2) + 1;
  ...
  Lines.prototype.getAxis = function(elem) {
    var pts;
    if (elem === "nav1") {
      pts = this.lgs("navs")[0].getBBox();
    }
    return { x: this.f(pts.x, 0) + 6, y: this.f(pts.y, 0) + 6 };
  };
  ...
  Lines.prototype.navsAngle = function() {
    ...
    angle = Snap.angle(pts[0].x, pts[0].y, pts[1].x, pts[1].y);
    console.log(pts, angle);
