...
class Calc {
  ...
  lines() {
    const data = store.get("data");
    store.set("points", this.dataToPoints(data));
  }
  ...
  candle() {
    const { points, stepX, stepY, raw: ohlcData } = store.mget([
      "points",
      "stepX",
      "stepY",
      "raw",
  ...
  ema(emaLen = 2) {
    ...
    emaParam.k2 = 1 - emaParam.k;
    const data = store.get("data");
    // console.log("EMA ...")
    let prev = data[0]; //
    emaData = data.map((d, idx) => {
      if (!idx) return d;

      const edata = d * emaParam.k + prev * emaParam.k2;

      prev = d;
      console.log(d, emaParam.k, prev, emaParam.k2);
