...
class Calc {
  ...
  nextPoint(diff, type) {
    ...
    store.push("points", [utils.f(nextX, 0), utils.f(nextY, 0)]);
   ...
   */
  candleShadow(linePoints, storeData, candle) {
    const { prevPoint } = linePoints;
    const { stepY, ohlc: { High, Low, Close } } = storeData;
    const { y: yAxis, xCenter, isWin } = candle;
    ...
    store.push("candleShadow", shadow);
  }
}
