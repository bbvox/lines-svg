...
class Chart {
  ...
  drawSma() {
    const { points } = store.mget(["points"], "sma");
    console.log("SMA DRAW : ", points);
    // index start from 0 & ignore last point
    let [pk, path, llen] = [0, "", points.length - 1];
    //toDo use while for simple loop
    for (; pk < llen; pk++) {
      path += this.getPath(points[pk], points[pk + 1]);
      this.svgDebug(points[pk]);
    ...
    this.svgPath(path, "stsma");
  }

  drawEma() {
    const { points } = store.mget(["points"], "ema");
    // index start from 0 & ignore last point
    let [pk, path, llen] = [0, "", points.length - 1];
    //toDo use while for simple loop
    for (; pk < llen; pk++) {
