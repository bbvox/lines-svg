...
class ChartBase {
  ...
  init() {
    ...
    // invert the chart start from right to left
    this.area.zeroX = this.area.width;
  ...
  setViewbox() {
    ...
    this.snap.attr({ viewBox: `0 0 ${w} ${h}` });
  }
  ...
  drawLabelsX() {
    const { labels, config } = store.mget(["labels", "config"]);
    const points = store.get("points", "line");

    const timePeriod = config && config.period ? config.period : cfg.timeUnit;
    ...
    labels.forEach((labelDate, idx) => {
      ...
      const label = utils.fDate(labelDate, timePeriod);
